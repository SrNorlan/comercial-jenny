...
async function creditHistory(id) {
  const { rows } = await db.query(
    'SELECT * FROM record_crediticio WHERE id_cliente = $1 ORDER BY fecha_compra DESC',
    [id],
  );
  return rows;
}
...
async function update(
  ...
  const { rows } = await db.query(
    `UPDATE persona SET nombre=$1, apellido=$2, cedula=$3, telefono=$4, distrito=$5, zona_residencia=$6, punto_referencia=$7, distancia=$8, casa=$9 WHERE id_persona=$10 AND tipo_persona='Cliente' RETURNING id_persona`,
    [
      nombre,
      apellido,
      cedula,
