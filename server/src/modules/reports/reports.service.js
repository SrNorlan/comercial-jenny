...
async function summary() {
  const [clients, products, sales, credit] = await Promise.all([
    ...
    db.query(
      'SELECT COUNT(*)::int AS total, COALESCE(SUM(existencia), 0)::int AS stock FROM productos',
    ),
    db.query(
      "SELECT COUNT(*)::int AS total, COALESCE(SUM(total_venta), 0) AS amount FROM venta WHERE fecha_venta >= date_trunc('month', CURRENT_DATE)",
    ),
