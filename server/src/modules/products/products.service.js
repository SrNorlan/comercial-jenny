...
async function list(category) {
  const query = category
    ? 'SELECT * FROM productos WHERE categoria = $1 ORDER BY id_producto DESC'
...
async function create({
  ...
  if (!marca || !color || !tipo || !categoria) {
    const error = new Error('Marca, color, tipo y categoría son requeridos.');
  ...
  if (precioCompra !== undefined && precioCompra !== null && precioCompra !== '')
    requireNonNegativeNumber(precioCompra, 'El precio de compra');
...
async function update(id, data) {
  ...
  if (precioCompra !== undefined && precioCompra !== null && precioCompra !== '')
    requireNonNegativeNumber(precioCompra, 'El precio de compra');
  const { rows } = await db.query(
    `UPDATE productos SET marca=$1, existencia=$2, precio_venta=$3, precio_compra=$4, color=$5, tipo=$6, categoria=$7, talla=$8, modelo=$9, clasificacion=$10, dimensiones=$11, unidad_medida=$12, fecha_vencimiento=$13 WHERE id_producto=$14 RETURNING *`,
    [
      marca,
      existencia,
      precioVenta || null,
...
async function updateStatus(id, estadoProducto) {
  ...
  const { rows } = await db.query(
    'UPDATE productos SET estado_producto=$1 WHERE id_producto=$2 RETURNING id_producto, estado_producto',
    [estadoProducto, id],
  );
  if (!rows.length) {
    const error = new Error('Producto no encontrado.');
