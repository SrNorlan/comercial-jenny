  ...
  return (
    <main className="login">
      <section className="login-intro">
        <p className="eyebrow">COMERCIAL JENNY / GESTIÓN COMERCIAL</p>
        <h1>
          Todo el negocio,
          <br />
          <em>en movimiento.</em>
        ...
        <p className="intro-copy">
          Una vista clara para vender mejor, cuidar el inventario y conocer a tus clientes.
        </p>
      ...
      <form onSubmit={submit} className="login-form">
        ...
        <p className="form-kicker">Acceso privado</p>
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) => setCredentials({ ...credentials, contrasena: event.target.value })}
          />
        </label>
        {error && <p className="error">{error}</p>}
        <button type="submit" disabled={loading}>
          {loading ? 'Validando...' : 'Entrar al sistema'} <span>→</span>
        </button>
      </form>
    </main>
  );
...
function getPage(view, data, user, reload, navigate) {
  ...
  if (view === 'pos')
    return (
      <PosPage products={data.products} clients={data.clients} user={user} onComplete={reload} />
    );
  if (view === 'installments')
    return <InstallmentsPage installments={data.installments} onPayment={reload} />;
  if (view === 'returns')
    return (
      <ReturnsPage
        sales={data.sales}
  ...
  if (view === 'sales')
    return (
      <RecordsPage
        ...
        rows={data.sales}
        columns={[
          { label: 'Venta', key: 'id', keys: ['id_venta', 'id'] },
          { label: 'Tipo', key: 'type', keys: ['tipo_venta', 'tipo'] },
          { label: 'Total', key: 'total', keys: ['total_venta', 'total'] },
        ]}
      />
    );
  if (view === 'purchases')
...
export default function App() {
  ...
  return (
    <AppLayout user={user} active={view} onNavigate={setView} onLogout={logout}>
      {getPage(view, data, user, reload, setView)}
    </AppLayout>
  );
}
